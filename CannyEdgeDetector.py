...
def gaussianSmoothing(image):
    """
    Applies  3x3 Gaussian Filter to the image by convolution operation
    :type image: object
    """
    ...
    for i in range(3, image.shape[0] - 3):
        for j in range(3, image.shape[1] - 3):
            sumGuassian = setGuassianFilterAtLocation(imageArray, i, j)
            gaussianArr[i][j] = sumGuassian

    return gaussianArr


def setGuassianFilterAtLocation(imageData, row, column):
    sum = 0
...
if __name__ == "__main__":
    ...
    image = cv2.imread('Images/rts1bq50.jpg', 0)
